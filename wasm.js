...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.18.10") {
        throw new Error(`Cannot start service: Host version "${"0.18.10"}" does not match binary version ${quote(binaryVersion)}`);
      }
      return;
    }
