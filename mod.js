...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.11.9") {
        throw new Error(`Cannot start service: Host version "${"0.11.9"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
  ...
  let buildOrServe = (args) => {
    ...
    if (plugins && plugins.length > 0) {
      ...
      handlePlugins(options, plugins, key, details).then(([requestPlugins, pluginRefs]) => buildOrServeContinue({
        ...args,
        key,
        details,
        requestPlugins,
